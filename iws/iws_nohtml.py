...
class InteractiveWeakSupervision:
    ...
    def next_candidate(self):
        ...
        if not self.init_completed:
            ...
            if self.counter >= self.maxiter:
                if not self.auto:
                    self.htmlwidget = 'Experiment completed'
    ...
    def show_candidate(self, idx):
        htmltext = '\n'
        htmltext += 'Please inspect this description carefully before looking at examples below:\n'
        htmltext += 'Please pay attention the term and the LF vote\n'
        htmltext += 'Description of heuristic: %s \n' % self.LFdescriptions[idx]
        htmltext += 'Is this labeling function better than chance? \n'
        self.htmlwidget = htmltext
        if self.numshow > 0:
            htmltext = 'Here are examples of where this heuristic might apply'
